...
(defun play-rom (file)
  ...
  (sdl:with-init (sdl:sdl-init-video sdl:sdl-init-audio)
    (setf *screen* (sdl:window 256 240 :bpp 24 :sw t :double-buffer t))
    (unwind-protect (run)
      (static-vectors:free-static-vector *frame*))))
    ...
    (loop do
         (let ((c-step (6502-step cpu (6502:get-byte (6502:immediate cpu))))
           ...
           (when (ppu-result-new-frame p-step)
             (let ((result (sdl-base::blit-surface (static-vector-pointer *frame*)
                                                   (sdl:fp *screen*)
                                                   (cffi:null-pointer)
                                                   (cffi:null-pointer))))
               (assert (zerop result)))
